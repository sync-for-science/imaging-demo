...
const PopoverTip3 = () => (
  <PopoverTip title="Fetch List of Imaging Studies from Imaging Data Server">
    <p>Next, the Imaging Demo App fetches a list of <code>ImagingStudy</code> resources via <code>GET /$imaging-data-server/ImagingStudy</code>. The resulting study metadata are used to draw a table of imaging studies, but no actual DICOM data have been fetched yet.</p>
  </PopoverTip>
);

const PopoverTip4 = () => (
  <PopoverTip title="Fetch DICOM Data for an Individual Study">
    <p>When a user clicks the <code>Download</code> button next to a specific study, the Imaging Demo App follows a series of links to retrieve the full DICOM study. Specifically, the app resolves the FHIR <code>ImagingStudy.endpoint</code>, and then the <code>Endpoint.address</code> to obtain the WADO-RS URL.</p>
  </PopoverTip>
);

const PopoverTip5 = () => (
  <PopoverTip title="Render a Study for the User to Review">
    <p>When a user clicks the <code>Review</code> button next to a specific study, the Imaging Demo App uses the <a href="https://cornerstonejs.org/" target="_blank">Cornerstone library</a> to render the study to the screen.</p>
  </PopoverTip>
);
